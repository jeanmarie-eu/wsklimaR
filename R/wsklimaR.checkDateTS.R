...
#' @export
#' @examples
#' wsklimaR.checkDateTS(timeserie,toPeriod,timeserietypeID)

wsklimaR.checkDateTS <- function(timeserie, timeserietypeID, fromPeriod, toPeriod, missingValue) {
 ...
 if (!is.null(timeserie)) {
  ...
  value = timeserie[,2]
   ...
   if ((timeserietypeID=="0")) {
         ...
         ts2.entire_date <- zoo(,(as.Date(seq(from =ts2.rangetime2[1], to =ts2.rangetime2[2], by = "day"))))
         ts3<-merge(x=ts2, y=ts2.entire_date)
         ts3[is.na(ts3)]<-missingValue
         ...
         ts2.entire_date <- zoo(,(as.Date(seq(from =ts2.rangetime2[1], to =ts2.rangetime2[2], by = "month"))))
         ts3<-merge(x=ts2, y=ts2.entire_date)
         ts3[is.na(ts3)]<-missingValue
       ...
       ts2.entire_date <- zoo(,(seq(from =ts2.rangetime2[1], to =ts2.rangetime2[2], by = "hour")))
       ...
       #rem: ts.date[13]: yearly normal
         ...
         ts2.entire_date <- zoo(,(as.Date(seq(from =ts2.rangetime2[1], to =ts2.rangetime2[2], by = "month"))+1))
         ts3<-merge(x=ts2, y=ts2.entire_date)
         ts3[is.na(ts3)]<-missingValue
       ...
       # Warning: 29th of February are not kept
         ...
         ts2<-zoo(ts[,5:dim(ts)[2]], as.Date(ts.date))
         ts2.rangetime2 <- range(((strptime(paste("1961","01","01",sep=""),"%Y%m%d",tz="GMT"))),((strptime(paste("1961","12","31",sep=""),"%Y%m%d",tz="GMT"))))
         ts2.entire_date <- zoo(,(as.Date(seq(from =ts2.rangetime2[1], to =ts2.rangetime2[2], by = "day"))+1))
         ts3<-merge(x=ts2, y=ts2.entire_date)
         ts3[is.na(ts3)]<-missingValue
   }
 ...
 df <- data.frame(
   ...
   if ((timeserietypeID=="0")) {
         ...
         # Normal Monthly: 1961-1990
         ts2.rangetime2 <- range(((strptime(paste("1961",fromMonth,fromDay,sep=""),"%Y%m%d",tz="GMT"))),((strptime(paste("1961",toMonth,toDay,sep=""),"%Y%m%d",tz="GMT"))))
         ts2.entire_date <- zoo(,(as.Date(seq(from =ts2.rangetime2[1], to =ts2.rangetime2[2], by = "month"))+1))
 ...
 df <- data.frame(
          ...
          value = rep(missingValue,length(attributes(ts2.entire_date)$index))
    )
  }
   return(df)
 }
